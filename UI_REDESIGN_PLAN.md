...
69. [done] Перезагрузить VMware test VM с frozen ISO `2b570a2e...`: VNC подтвердил ровно три GRUB entry, installer default при отсутствии локальной `kernel` label и безопасный autostart до выбора `/dev/sda` без disk write. Conditional installed-disk default дополнительно покрыт BIOS/UEFI QEMU gate.
